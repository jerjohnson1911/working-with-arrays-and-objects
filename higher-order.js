...
//Code Here
let evenNumbers  = mixedNumbers.filter(function(mixedNumbers){
  return mixedNumbers % 2 === 0;
});
...
//Code Here
let totalPopulation = populations.reduce((acc, cur) => {
  return acc + cur
...
//Code Here
// let myStrongest = monstersInYourPocket.filter(function(e,i){
//   return e.CP > 200
...
let orderTotals = orders.map((e,i) => e.price * (1 + e.tax) )
...
let bobsTotal = purchases.reduce(function(acc, cur){
  if(cur.owner === "Bob"){
    acc += cur.price
...
// let bobsTotal = purchases.reduce((acc, cur) => acc.owner.Bob + cur.price, 0) 
